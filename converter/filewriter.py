...
class OutFile:
    ...
    def create_datasets(self, orient_time, accelerometer, magnetometer,
                        current, temp_time, temperature,
    ...
    def write_data(self, orient_time=None, accelerometer=None,
                   magnetometer=None, current=None,
                   temp_time=None, temperature=None, light=None,
...
class Csv:
    ...
    def _create_datasets(self, orient_time, accelerometer, magnetometer,
                         current, temp_time, temperature,
        ...
        if accelerometer is not None or magnetometer is not None:
            self.orient_file = path.join(self.out_path, self.prefix +
                                         '_accelmag.csv')
        ...
        if current is not None:
            self.current_file = path.join(self.out_path, self.prefix +
                                          '_current.csv')
            with open(self.current_file, 'w') as fid:
                fid.write(TIME_HEADER_FORMAT.format(time_header))
        if bearing is not None:
            self.bearing_file = path.join(self.out_path,
                                          self.prefix + '_bearing.csv')
            with open(self.bearing_file, 'w') as fid:
                fid.write('{},Bearing (degrees)\n'.format(time_header))
        if temperature is not None:
            self.temperature_file = path.join(self.out_path,
                                              self.prefix + '_temperature.csv')
            with open(self.temperature_file, 'w') as fid:
                fid.write('{},Temperature (C)\n'.format(time_header))
        if light is not None:
            self.light_file = path.join(self.out_path,
                                        self.prefix + '_light.csv')
            with open(self.light_file, 'w') as fid:
                fid.write('{},Light (%)\n'.format(time_header))
        if pressure is not None:
            self.pressure_file = path.join(self.out_path,
                                           self.prefix + '_pressure.csv')
    ...
    def write_data(self, orient_time=None, accelerometer=None,
        ...
        if self.is_first_page:
            self.start_time = np.min([
                start_time[0]
                for start_time in [orient_time, temp_time, pressure_time]
                if start_time is not None])
            self._create_datasets(orient_time, accelerometer, magnetometer,
                                  current, temp_time, temperature,
                                  light, pressure_time, pressure, bearing)
            self.is_first_page = False

        if accelerometer is not None or magnetometer is not None:
            accel_length = (accelerometer.shape[1]
                            if accelerometer is not None else 0)
            mag_length = (magnetometer.shape[1]
                          if magnetometer is not None else 0)
            ...
            with open(self.orient_file, 'a') as fid:
                for i in range(orient_length):
                    ...
                    if accelerometer is not None:
                        fid.write(',{:0.4f},{:0.4f},{:0.4f}'.format(
                            *accelerometer[:, i]))
                    if magnetometer is not None:
                        fid.write(',{:0.2f},{:0.2f},{:0.2f}'.format(
                            *magnetometer[:, i]))
                    fid.write('\n')
        ...
        if temperature is not None:
            with open(self.temperature_file, 'a') as fid:
                for i in range(temperature.shape[0]):
                    time = self.make_time_string(temp_time[i])
                    fid.write('{}'.format(time))
    ...
    def make_time_string(self, posix_time):
        ...
        # The float functions converts the value from a np array to a
        # python float without it the number in the output file is in
        # square braces
        if self.time_format == 'iso8601':
            return datetime.datetime.utcfromtimestamp(
                float(posix_time)).strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3]
        elif self.time_format == 'legacy':
            return datetime.datetime.utcfromtimestamp(
                float(posix_time)).strftime('%Y-%m-%d,%H:%M:%S.%f')[:-3]
...
class Hdf5:
    ...
    def _create_datasets(self, orient_time, accelerometer, magnetometer,
                         current, temp_time, temperature,
                         light, pressure_time, pressure, bearing):
        full_path = path.join(self.out_path, self.prefix + '.hdf5')
        with h5py.File(full_path, 'w') as hdf_file:
            if orient_time is not None:
                hdf_file.create_dataset('orient_time',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float64',
                                        compression='gzip')
            ...
            if accelerometer is not None:
                hdf_file.create_dataset('accelerometer',
                                        (3, 0),
                                        maxshape=(3, None),
                                        dtype='float32',
                                        compression='gzip')
            ...
            if magnetometer is not None:
                hdf_file.create_dataset('magnetometer',
                                        (3, 0),
                                        maxshape=(3, None),
                                        dtype='float32',
                                        compression='gzip')
            ...
            if current is not None:
                hdf_file.create_dataset('current',
                                        (4, 0),
                                        maxshape=(4, None),
                                        dtype='float32',
                                        compression='gzip')
            ...
            if temp_time is not None:
                hdf_file.create_dataset('temperature_time',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float64',
                                        compression='gzip')
            ...
            if temperature is not None:
                hdf_file.create_dataset('temperature',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float32',
                                        compression='gzip')
            ...
            if pressure_time is not None:
                hdf_file.create_dataset('pressure_time',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float64',
                                        compression='gzip')
            ...
            if pressure is not None:
                hdf_file.create_dataset('pressure',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float32',
                                        compression='gzip')
            ...
            if bearing is not None:
                hdf_file.create_dataset('bearing',
                                        (0,),
                                        maxshape=(None,),
                                        dtype='float32',
                                        compression='gzip')

    def write_data(self, orient_time=None,
                   accelerometer=None, magnetometer=None, current=None,
                   temp_time=None, temperature=None, light=None,
                   pressure_time=None, pressure=None, bearing=None):
        # Create the data sets if they haven't been already
        if self.is_first_page:
            self.start_time = np.min(
                [start_time[0] for start_time in
                 [orient_time, temp_time, pressure_time]
                 if start_time is not None])
            self._create_datasets(orient_time, accelerometer, magnetometer,
                                  current, temp_time, temperature,
                                  light, pressure_time, pressure, bearing)
            self.is_first_page = False
        full_path = path.join(self.out_path, self.prefix + '.hdf5')
        with h5py.File(full_path, 'a') as hdf_file:
            if orient_time is not None:
                data_length = orient_time.shape[0]
                dset_length = hdf_file['orient_time'].shape[0]
                size = dset_length + data_length
                hdf_file['orient_time'].resize((size,))
                hdf_file['orient_time'][dset_length:size] = orient_time

            if accelerometer is not None:
                data_length = accelerometer.shape[1]
                dset_length = hdf_file['accelerometer'].shape[1]
                size = dset_length + data_length
                hdf_file['accelerometer'].resize((3, size))
                hdf_file['accelerometer'][:, dset_length:size] = accelerometer

            if magnetometer is not None:
                data_length = magnetometer.shape[1]
                dset_length = hdf_file['magnetometer'].shape[1]
                size = dset_length + data_length
                hdf_file['magnetometer'].resize((3, size))
                hdf_file['magnetometer'][:, dset_length:size] = magnetometer

            if current is not None:
                data_length = current.shape[1]
                dset_length = hdf_file['current'].shape[1]
                size = dset_length + data_length
                hdf_file['current'].resize((4, size,))
                hdf_file['current'][:, dset_length:size] = current

            if bearing is not None:
                data_length = bearing.shape[0]
                dset_length = hdf_file['bearing'].shape[0]
                size = dset_length + data_length
                hdf_file['bearing'].resize((size,))
                hdf_file['bearing'][dset_length:size] = bearing

            if temp_time is not None:
                data_length = temp_time.shape[0]
                dset_length = hdf_file['temperature_time'].shape[0]
                size = dset_length + data_length
                hdf_file['temperature_time'].resize((size,))
                hdf_file['temperature_time'][dset_length:size] = temp_time

            if temperature is not None:
                data_length = temperature.shape[0]
                dset_length = hdf_file['temperature'].shape[0]
                size = dset_length + data_length
                hdf_file['temperature'].resize((size,))
                hdf_file['temperature'][dset_length:size] = temperature

            if light is not None:
                data_length = light.shape[0]
                dset_length = hdf_file['light'].shape[0]
                size = dset_length + data_length
                hdf_file['light'].resize((size,))
                hdf_file['light'][dset_length:size] = light

            if pressure_time is not None:
                data_length = pressure_time.shape[0]
                dset_length = hdf_file['pressure_time'].shape[0]
                size = dset_length + data_length
                hdf_file['pressure_time'].resize((size,))
                hdf_file['pressure_time'][dset_length:size] = pressure_time

            if pressure is not None:
                data_length = pressure.shape[0]
                dset_length = hdf_file['pressure'].shape[0]
                size = dset_length + data_length
                hdf_file['pressure'].resize((size,))
                hdf_file['pressure'][dset_length:size] = pressure
